...
/* 
В втором тесте тестируется соответствие ожидаемого и получаемого на клиенте Типа результата:app.company_info_public. Используется CodeceptJS + NodeJS + Selenium WebDriver.
*/ 
Scenario('REST API test ', function*(I){
 unirest.get('http://www.tender.pro/api/_company.info_public.json?id=198').end((responseFromApi) => {
	...
	let expectedAttributes =["address_legal","title_full","anno_short", "rating","is_seller_producer","country_id", "fax", "is_type_seller","address","id","anno","type_name","country_name","phone","seller_type_name","kpp","inn","site","title"];
